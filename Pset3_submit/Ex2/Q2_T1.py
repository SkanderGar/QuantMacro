...
import Agent_T1 as A
from numba import jit
from Agent_T1 import U
...
V = []
for agent in Agents:
    v1 = U((1-agent.tho)*agent.eta*store_h+agent.y-store_a+Av_T1, store_h, agent.sigma, agent.kappa, agent.nu) 
    v2 = agent.beta*(0.5*U((1-agent.tho)*(agent.eta+agent.eps)*store_h_p+(1+agent.r)*store_a+Av_T2, store_h_p, agent.sigma, agent.kappa, agent.nu)+ 
                     0.5*U((1-agent.tho)*(agent.eta-agent.eps)*store_h_p+(1+agent.r)*store_a+Av_T2, store_h_p, agent.sigma, agent.kappa, agent.nu))
    V.append(v1+v2)
...
np.savetxt('variables_T1.txt',data)
np.savetxt('Market_T1.txt',store_m_c)
