...
class agent1:
    ...
    def __init__(self, N_a, N_s=2, sigma_y = 0.5, gamma = 0.7, rho = 0.06, Sig = 5, C_ = 1, r = 0.04, U2 = 1, B=1):
        self.beta = 1/(1+rho)
        self.Sig = Sig
        self.C_ = C_
        self.r = r
        ...
        self.N_s = N_s
        self.N_a = N_a
        self.Tr, self.Y_grid = self.markov_Tr(self.N_s,  Sig_y = sigma_y, gamma=gamma)
        max_a = self.Y_grid[-1]/self.r
        if B==1:
            min_a = -(self.Y_grid[0]/self.r)*0.7 #just to have a smoother value function
        else:
            min_a = 0
    ...
    def problem(self, start = None, Tol = 10**(-6)):
        if start == None:
